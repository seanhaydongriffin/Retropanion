...
$system_dict.Add("Atari 2600", "2")
$system_dict.Add("Atari 5200", "5")
$system_dict.Add("Atari 7800", "6")
$system_dict.Add("Atari 8-bit Family", "8")
$system_dict.Add("Atari Jaguar", "J")
$system_dict.Add("Atari Jaguar CD", "37")
$system_dict.Add("Atari Lynx", "L")
$system_dict.Add("Atari ST / TT / Falcon", "S")
...
Local $system = "Atari 8-bit Family" ; $CmdLine[1]
...
for $page_num = $page_start_page to $end_page
	...
	for $page_num2 = 1 to 50
		...
		Local $iPID = Run('curl.exe -s -k http://www.atarimania.com/list_games_atari-400-800-xl-xe-p_total-page-step-letter_' & $page_id[$page_num - 1] & '-' & $page_num2 & '-200-' & $page_letter[$page_num - 1] & '_' & $system_dict.Item($system) & '_G.html', @ScriptDir, @SW_HIDE, $STDOUT_CHILD)
		ProcessWaitClose($iPID)
		Local $html = StdoutRead($iPID)
		Local $arr = StringRegExp($html, ".*(?U)<A HREF=""(.*)"" CLASS=""LienNoirGras"">(.*)<\/A>", 3)
		...
		for $i = 0 to (UBound($arr) - 1) step 2
			...
			Local $iPID = Run('curl.exe -s -k http://www.atarimania.com/' & $arr[$i + 0], @ScriptDir, @SW_HIDE, $STDOUT_CHILD)
			ProcessWaitClose($iPID)
			Local $html = StdoutRead($iPID)
			Local $arr2 = StringRegExp($html, "(?U)HREF=""8bit/boxes/hi_res/(.*)"">", 3)

			for $j = 0 to (UBound($arr2) - 1)

				Local $img_url = "http://www.atarimania.com/8bit/boxes/hi_res/" & $arr2[$j]
				ConsoleWrite("Found " & $img_url & @CRLF)

				; find the next available filename

				Local $game_art_num = 1

				While FileExists($emulator_folder & "\Box\" & $arr[$i + 1] & "-" & $game_art_num & ".jpg") = True

					$game_art_num = $game_art_num + 1
				WEnd

				InetGet($img_url, $emulator_folder & "\Box\" & $arr[$i + 1] & "-" & $game_art_num & ".jpg")
			Next
		Next
	Next
