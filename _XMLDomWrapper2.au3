...
Func _XMLSaveXML($objDoc,$strXMLFile)
...
;										1 = No object passed
;===============================================================================
Func _XMLGetValue($objDoc, $strXPath, $single = True, $sqlite_friendly = False, $removal_str = "")
	If not IsObj($objDoc) then
		...
		If $objNodeList.length > 0 Then
			...
			For $objNode In $objNodeList
				If $objNode.hasChildNodes () = False Then
					...
					$objnode_val = $objNode.nodeValue

					if StringLen($removal_str) > 0 Then

						$objnode_val = StringReplace($objnode_val, $removal_str, "")
					EndIf

					if $sqlite_friendly = True Then

						$objnode_val = StringReplace($objnode_val, "'", "''")
					EndIf

					_XMLArrayAdd($arrResponse, $objnode_val)
				Else
					...
					For $objNodeChild In $objNode.childNodes ()
						...
						If $objNodeChild.nodeType = $NODE_CDATA_SECTION Then
							...
							$objnodechild_data = $objNodeChild.data

							if StringLen($removal_str) > 0 Then

								$objnodechild_data = StringReplace($objnodechild_data, $removal_str, "")
							EndIf

							if $sqlite_friendly = True Then

								$objnodechild_data = StringReplace($objnodechild_data, "'", "''")
							EndIf

							_XMLArrayAdd($arrResponse, $objnodechild_data)
							_DebugWrite("GetValue>CData:" & $objNodeChild.data)

						ElseIf $objNodeChild.nodeType = $NODE_TEXT Then

							$objnodechild_text = $objNodeChild.Text

							if StringLen($removal_str) > 0 Then

								$objnodechild_text = StringReplace($objnodechild_text, $removal_str, "")
							EndIf

							if $sqlite_friendly = True Then

								$objnodechild_text = StringReplace($objnodechild_text, "'", "''")
							EndIf

							_XMLArrayAdd($arrResponse, $objnodechild_text)
...
;									2 = No object
;===============================================================================
Func _XMLGetAttrib($objDoc,$strXPath, $strAttrib, $strQuery = "", $single = True, $sqlite_friendly = False)
;	ConsoleWrite('@@ Debug(' & @ScriptLineNumber & ') : $strXPath = ' & $strXPath & @crlf & '>Error code: ' & @error & @crlf) ;### Debug Console
	If not IsObj($objDoc) then
...
;			$arrResponse = $objAttr
			...
			if $sqlite_friendly = True Then
				...
				_XMLArrayAdd($arrResponse, StringReplace(StringReplace($objAttr, "'", "''"), """", """"""))
			Else

				_XMLArrayAdd($arrResponse, $objAttr)
			EndIf

;			_XMLArrayAdd($arrResponse, $objAttr)
			_DebugWrite("RET>>" & $objAttr)
;			ConsoleWrite($objAttr)
		Next
;		_ArrayDisplay($arrResponse)
...
;===============================================================================
Func _XMLGetChildText($objDoc, $strXPath)
	If not IsObj($objDoc) then
...
;===============================================================================
Func _XMLGetPath($objDoc, $strXPath)
	If not IsObj($objDoc) then
